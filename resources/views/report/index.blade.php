...
<div class="container mx-auto px-4">
    ...
    <div class="flex flex-wrap justify-end items-center mb-5 w-full gap-4">
        <!-- Add Button -->
        <button class="bg-green-500 text-white px-3 py-2 rounded-md hover:bg-green-600" 
                onclick="openModal()">
            Add Report
        </button>

        <div class="relative">
    <button id="filterButton" class="bg-yellow-500 text-white px-3 py-2 rounded-md hover:bg-yellow-600 w-full" onclick="toggleDropdown()"> 
        Filter by Category
    </button>

    <ul id="filterDropdown" class="absolute z-50 bg-white border rounded-md shadow-lg mt-1 hidden w-full">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-black-500 dark:text-black-400">
            <thead class="text-xs text-black-700 uppercase bg-gray-200 text-gray-700">
                <tr>
            ...
            <tbody id="reportTableBody">
                @foreach ($reports as $report)
                    <tr class="bg-white border-b">
                        ...
                        <button onclick="openEditModal({{ $report }})" class="bg-yellow-500 text-white px-3 py-2 rounded-md hover:bg-yellow-600">
...
<!-- Add Report Modal -->
<div id="addReportModal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 hidden">
    <div class="bg-white p-6 rounded-lg shadow-lg w-[1500em] max-w-auto max-w-4xl mx-4 sm:mx-auto overflow-y-auto max-h-[80vh] relative"> 
        <!-- Close Button -->
        <button onclick="closeModal()" class="absolute top-4 right-4 text-red-600 text-xl font-bold">
            ✖
        </button>

        <h5 class="text-xl font-bold mb-4 text-center">Add New Report</h5>
