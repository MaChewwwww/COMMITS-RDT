...
@section('content')
    ...
    <div class="flex flex-row justify-end pb-6 pr-16 text-white gap-x-5">
        <button onclick=openModal() class="px-4 py-2 bg-red-800 rounded-lg">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-6">
                <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
              </svg>              
        </button>
        <button onclick=printDiv() class="px-4 py-2 bg-red-800 rounded-lg">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-6">
            ...
            <table class="border border-collapse border-black table-auto text-start">
                ...
                <tbody>
                    ...
                    @foreach ($services as $service)
                        <tr>
                            <td class="w-64 pl-5 border border-black 
                            {{-- add padding only to specific cells --}}
                            {{ in_array($loop->index, 
                            [2, 7, 13, 15, 16, 17, 18, 19, 21, 22, 23, 
                            24, 25, 27, 28, 29, 31, 32, 33, 35,
                            36, 37, 38, 39, 42, 44, 45, 46]) ? 'pl-7' : 
                            (in_array($loop->index, [3, 4, 5, 6, 8, 9, 10, 11, 12]) ? 'pl-9' : '') }}">{{ $service['name'] }}</td>
            ...
            <table id="table2" class="mt-5 font-semibold w-[60%]">
                <tbody>
                    ...
                    <tr class="text-center">
                        ...
                        <td class="text-center border border-black bg-amber-400">
                            <div class="flex flex-row justify-center gap-x-1">
                                M = <p id="onlineConsultMaleOut"></p></td>
                            </div>
                        <td class="text-center border border-black bg-amber-400">
                            <div class="flex flex-row justify-center gap-x-1">
    ...
    <div id="addReportModal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-gray-900 bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-[80vh] max-w-auto max-w-4xl mx-4 sm:mx-auto overflow-y-auto max-h-[80vh] relative"> 
            ...
            <form id="addReportForm" class="space-y-4">
                ...
                <button id="submitBtn" onclick="saveEdit()" type="button" class="w-full p-3 text-white bg-green-500 rounded-md hover:bg-green-600">
                    Save
                </button>
            </form>
        </div>
...
<script>
    ...
    function saveEdit() {
        ...
        onlineConsultTotalOut.innerHTML = parseInt(totalOnlineConsultsMale.value) + parseInt(totalOnlineConsultsFemale.value);
        grandTotalMaleOut.innerHTML = parseInt(totalF2FConsultsMale.value) + parseInt(totalOnlineConsultsMale.value);
        grandTotalFemaleOut.innerHTML = parseInt(totalF2FConsultsFemale.value) + parseInt(totalOnlineConsultsFemale.value);
